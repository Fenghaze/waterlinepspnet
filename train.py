...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda,aux_branch):
    ...
    torch.save(model.state_dict(),
           'logs/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % ((epoch + 1), total_loss / (epoch_size + 1), val_toal_loss/(epoch_size_val+1)))

    if val_toal_loss/(epoch_size_val+1) < best_val_loss:
        best_val_loss = val_toal_loss/(epoch_size_val+1)
...
if __name__ == "__main__":
    ...
    #-------------------------------#
    Cuda = True
    model = PSPNet(num_classes=NUM_CLASSES, backbone=backbone, downsample_factor=downsample_factor, pretrained=True, aux_branch=aux_branch, skip_upsample=True).train()
    #-------------------------------------------#
    #   权值文件的下载请看README
    #   权值和主干特征提取网络一定要对应
    #-------------------------------------------#
    ...
    # 打开数据集的txt
    with open(r"D:\Deeplearning\data\ships1\ImageSets\Segmentation\train.txt","r") as f:
        train_lines = f.readlines()

    # 打开数据集的txt
    with open(r"D:\Deeplearning\data\ships1\ImageSets\Segmentation\val.txt","r") as f:
